...
func NextDate(now time.Time, date string, repeat string) (string, error) {
	...
	case "d": // дни
		return repeatDateDay(now, parsedDate, rules)

	case "w": // дни недели
		return repeatDateWeek(now, parsedDate, rules)

	case "m": // дни месяца
		return repeatDateMonth(now, parsedDate, rules)

	case "y": // год
		return repeatDateYear(now, parsedDate)
	}
	return "", errors.New("неправильный формат повтора")
}

func repeatDateDay(now time.Time, parsedDate time.Time, rules []string) (string, error) {
...
func repeatDateWeek(now time.Time, parsedDate time.Time, rules []string) (string, error) {
	...
	if parsedDate.Before(now) {
		...
		for i := 0; i < 999; i++ {
			dateUnix += day
			for _, v := range chosenWeekdaysInt {
				caseWResult := time.Unix(dateUnix, 0)
				if caseWResult.Weekday() == weekdaysMap[v] {
					return caseWResult.Format("20060102"), nil
				}
			}
...
func repeatDateMonth(now time.Time, parsedDate time.Time, rules []string) (string, error) {
...
func repeatDateYear(now time.Time, parsedDate time.Time) (string, error) {
